...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.boltVersion) {
      body += `- AllAble: ${data.environmentInfo.boltVersion}\n`;
    }
  ...
  body += '---\n*Submitted via AllAble bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'stackblitz-labs/AllAble';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'AllAble-bug-reporter',
    });
