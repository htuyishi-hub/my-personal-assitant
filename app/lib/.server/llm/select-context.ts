...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyAllAbleActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyAllAbleActions(content);
...
---
  ...
  if (totalFiles == 0) {
    throw new Error(`AllAble failed to select files`);
  }
